...
class Solver {
	...
	void Solve(int l, QMatrix Q, double[] p_, byte[] y_,
		   ...
		   SolutionInfo si, int shrinking, boolean showOutput)
	{
		this.l = l;
		this.Q = Q;
		QD = Q.get_QD();
...
final class Solver_NU extends Solver
{
	...
	{
		this.si = si;
		super.Solve(l,Q,p,y,alpha,Cp,Cn,eps,si,shrinking,true);
	}
...
public class svm {
	...
	private static void solve_c_svc(svm_problem prob, svm_parameter param,
					...
					double Cp, double Cn, boolean showOutput)
	{
		...
		s.Solve(l, new SVC_Q(prob,param,y), minus_ones, y,
			alpha, Cp, Cn, param.eps, si, param.shrinking,showOutput);
					...
					boolean showOutput)
	{
		...
		s.Solve(l, new ONE_CLASS_Q(prob,param), zeros, ones,
			alpha, 1.0, 1.0, param.eps, si, param.shrinking,showOutput);
	}
					...
					boolean showOutput)
	{
		...
		s.Solve(2*l, new SVR_Q(prob,param), linear_term, y,
			alpha2, param.C, param.C, param.eps, si,
		...
		double Cp, double Cn, boolean showOutput)
	{
		...
		switch(param.svm_type)
		{
			case svm_parameter.C_SVC:
				solve_c_svc(prob,param,alpha,si,Cp,Cn,showOutput);
				break;
			case svm_parameter.NU_SVC:
				solve_nu_svc(prob,param,alpha,si,showOutput);
				break;
			case svm_parameter.ONE_CLASS:
				solve_one_class(prob,param,alpha,si,showOutput);
				break;
			case svm_parameter.EPSILON_SVR:
				solve_epsilon_svr(prob,param,alpha,si,showOutput);
				break;
			case svm_parameter.NU_SVR:
				solve_nu_svr(prob,param,alpha,si,showOutput);
				break;
		}
	...
	private static void svm_binary_svc_probability(svm_problem prob,
			...
			double[] probAB, boolean showOutput)
	{
		int i;
		int nr_fold = 5;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			else
			{
				...
				svm_model submodel = svm_train(subprob,subparam,showOutput);
				for(j=begin;j<end;j++)
				{
	...
	public static svm_model svm_train(svm_problem prob,
			svm_parameter param, boolean showOutput)
	{
		   ...
		   param.svm_type == svm_parameter.NU_SVR)
		{
			...
			decision_function f = svm_train_one(prob,param,0,0,showOutput);
		...
		else
		{
			...
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					...
					f[p] = svm_train_one(sub_prob,param,weighted_C[i],weighted_C[j],showOutput);
					for(k=0;k<ci;k++)
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target)
	{
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			svm_model submodel = svm_train(subprob,param,true);
			if(param.probability==1 &&
	...
	public static double getDistance(svm_model model, double[] decision_values) {
		...
		if (model.param.svm_type == svm_parameter.C_SVC
			...
			double w_module = vectorModule(w);
			double decision_values_module = vectorModule(decision_values);
		
			double distance = decision_values_module / w_module;
			return distance;
		}
		else {
			return -1;
	...
	private static double vectorModule(double[] vector) {
